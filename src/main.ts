...
//@ts-ignore
import gr from "../static/images/valley.png";
import { Simulation, SIMULATION_EXAMPLE } from './models/json-model';
//! VITE IMAGE IMPORTS

const TEMP_DATA = JSON.stringify(SIMULATION_EXAMPLE);
const DATA_LOAD: Simulation = JSON.parse(TEMP_DATA)

//! TODOS
//TODO: FIX OBJECTS OVERLAPPING
//TODO: LOAD SIMULATION FROM JSON, OR TEMPORARY OBJECT IN JSON-MODEL.TS
//TODO: PRODUCE JSON
//TODO: MAKE THE SUN SPIN AND PANELS LOOK AT IT
//! TODOS
...
let grid: Grid = {
	mwps: 0,
	size: {
		x: DATA_LOAD.map.grid.x,
		y: DATA_LOAD.map.grid.y
	},
	buildings: [],
	panels: [],
	tiles: [],
	...
	placeBuilding: function (): void {
		let widthy = +(document.getElementById("widthy") as HTMLInputElement).value;
		let depthy = +(document.getElementById("depthy") as HTMLInputElement).value;
		let floory = +(document.getElementById("floory") as HTMLInputElement).value;
		if (depthy + floory + widthy == 0) alert("ERROR: INVALID VALUES"); else placeBuilding({
...
function cursorPosition() {
	...
	if (col.height > 0.5 || colombettiriccardostuart.r + colombettiriccardostuart.g + colombettiriccardostuart.b > 2) {
		...
		grid.selectionBuilding = PLACEHOLDERMESH;
	}
...
if (graphicsSettings.ground) {
	...
	loader.load(
		...
		function ( object ) {
			console.log(object);
			let bigguh = 100 + Math.max(grid.size.x, grid.size.y)
			object.scale.set(bigguh, bigguh, bigguh);
			object.rotation.x = -1.566666;
			object.position.y = -5;
						
			const sky = new THREE.TextureLoader().load( gr );
			object.children[0].material = new THREE.MeshStandardMaterial({map: sky});

			object.position.x = grid.tiles[halfX][halfY].position.x - 50;
			object.position.z = grid.tiles[halfX][halfY].position.z + 50;
			scene.add( object );
		},
...
let stats = {
	...
	//@ts-ignore
stats.name.innerHTML = DATA_LOAD.name;
//@ts-ignore
stats.owner.innerHTML = DATA_LOAD.owner;
//@ts-ignore
stats.created.innerHTML = DATA_LOAD.creation;

setInterval(() => {
	//@ts-ignore
	stats.time.innerHTML = new Date().getHours() + ":" + new Date().getMinutes() + ":" + new Date().getSeconds() + " - " + new Date().getDay() + "/" + new Date().getMonth() + "/" + new Date().getFullYear();
	grid.mwhProduction();
	//@ts-ignore
